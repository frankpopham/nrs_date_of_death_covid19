...
download.file("https://www.nrscotland.gov.uk/files//statistics/covid19/covid-deaths-data-week-17.xlsx", destfile="excel.xlsx")
df <- read.xlsx("excel.xlsx", sheetIndex=19 , 
                startRow=6, endRow=51, header=FALSE)
df <- df %>%
  select(-X3) %>%
  filter(!is.na(X2)) %>%
  rename(Date=X1) 
